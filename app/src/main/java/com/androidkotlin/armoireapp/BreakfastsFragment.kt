...
import com.androidkotlin.construct.*
import com.androidkotlin.models.Recipes
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.*

class BreakfastsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val v = inflater.inflate(R.layout.fragment_breakfasts, container, false)
        auth = FirebaseAuth.getInstance()
        dataBase= FirebaseDatabase.getInstance()
        recipeReciclerView = v.findViewById(R.id.recicler_view_recipes_breakfasts)
        recipeReciclerView.layoutManager = LinearLayoutManager(this.context)
        recipeReciclerView.setHasFixedSize(true)
        RecyclerViewAdapterBF(this.context!!,mNames,mImageUrls,mDescription,mRecipe)
        recipeReciclerView.layoutManager = GridLayoutManager(this.context,1)
        recipeArrayList = arrayListOf<Recipes>()
        getRecipeBreakfastsData()
        return v
    }
    private fun getRecipeBreakfastsData() {
        ...
        dbReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    recipeReciclerView.adapter = RecipeAdapter_Breakfast(recipeArrayList)
                }
            }
